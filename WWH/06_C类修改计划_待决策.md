...
- **推荐方案 A（我推荐）**：① 输出《SSE 协议规范》文档（WWH/07 或 docs/）：事件类型表、payload 结构、时序、错误约定；② 代码层清理：`SSEEventType.MESSAGE` 标记 deprecated（保留兼容）、`text` content 统一为 str（dict 场景走专用事件）；③ 在协议文档中明确 DONE 的三种到达路径（正常/取消/超时失败）。
  - 工作量：S；风险：低。
