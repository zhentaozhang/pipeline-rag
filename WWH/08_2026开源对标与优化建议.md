...
- **状态**：✅ 已实施（2026-08-15）——连接器抽象 `DocumentConnector` + **S3 连接器**（扫描 bucket/prefix → 过滤类型 → 下载 → 触发文档流水线，`document.import_s3` Celery 任务，`CONNECTOR_S3_*` 配置）；Confluence/网页爬虫可按同一抽象扩展；3 个单测
